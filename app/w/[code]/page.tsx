...
export default function WorkshopParticipant(){
  ...
  if(stage==="report"){
    ...
    return <main className="reportPage">
      ...
      <section className="reportHero"><div className="reportHeroMain"><span className="reportEyebrow">بصمتك التعبيرية</span><div className="fingerprintCode">{result.fingerprint}</div><div className="reportPerson"><h1>{name}</h1><p>{org || "مشارك في الورشة"}</p></div></div><div className="fingerprintSummary"><span>البعد الأكثر حضورًا</span><b>{styleMeta[primary.code].title}</b><strong>{primary.mean.toFixed(1)}<em>/5</em></strong></div></section>
