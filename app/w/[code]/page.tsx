...
export default function WorkshopParticipant(){
  ...
  if(stage==="report"){
    ...
    return <main className="reportPage">
      <header className="reportTop"><div className="platformBrand compact"><div className="differenceMark small"><span>ت</span></div><div><b>التمايز</b><small>تقرير بصمة التعبير</small></div></div><div className="reportActions"><button className="outlineButton" onClick={()=>window.print()}><Download size={16}/> PDF</button><button className="outlineButton" onClick={()=>navigator.share?.({title:"بصمتي التعبيرية",text:"بصمتي التعبيرية في منصة التمايز: "+result.fingerprint})}><Share2 size={16}/> مشاركة</button></div></header>
      <section className="reportHero"><div><span className="reportEyebrow">بصمتك التعبيرية</span><div className="fingerprintCode">{result.fingerprint}</div><h1>{name}</h1><p>{org}</p></div><div className="fingerprintSummary"><span>النمط الأعلى</span><b>{styleMeta[primary.code].title}</b><small>متوسط {primary.mean.toFixed(1)} من 5</small></div></section>
